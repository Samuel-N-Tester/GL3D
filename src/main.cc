...
std::shared_ptr<SDL_Window> InitWorld() {
	...
	_window = SDL_CreateWindow("Shader Example", SDL_WINDOWPOS_CENTERED,
...
ApplicationMode ParseOptions(int argc, char ** argv) {
	...
	desc.add_options()("help", "print this help message")("translate",
			"Show translation example (default)")("rotate",
			"Show rotation example")("scale", "Show scale example");
...
int main(int argc, char ** argv) {
	...
	auto manager = game_world->asset_manager;
	// Add the main event loop
	SDL_Event event;
	while (SDL_WaitEvent(&event)) {
		switch (event.type) {
		case SDL_MOUSEMOTION:
			manager->rotateCamera('y', event.motion.xrel);
			manager->rotateCamera('x', event.motion.yrel);
			break;
		case SDL_QUIT:
			SDL_Quit();
			break;
